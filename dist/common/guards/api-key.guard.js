...
function isAdminRequest(headers, configService) {
    const expectedKey = configService.get('ADMIN_API_KEY');
    if (!expectedKey)
        return false;
...
let ApiKeyGuard = class ApiKeyGuard {
    ...
    canActivate(context) {
        ...
        if (!expectedKey) {
            throw new common_1.UnauthorizedException('Admin API key is not configured on the server.');
        }
